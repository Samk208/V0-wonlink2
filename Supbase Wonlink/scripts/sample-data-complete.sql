...
-- Show a quick data overview for verification
SELECT 
    '=== QUICK DATA VERIFICATION ===' as section,
    '' as details
UNION ALL
SELECT 
    'PROFILES' as section,
    name || ' (' || role || ') - ' || email as details
FROM profiles
ORDER BY role, name
UNION ALL
SELECT 
    'CAMPAIGNS' as section,
    title || ' - $' || budget::text || ' (' || status || ')' as details
FROM campaigns
ORDER BY created_at
UNION ALL
SELECT 
    'APPLICATIONS' as section,
    'Campaign: ' || c.title || ' | Influencer: ' || p.name || ' | Status: ' || ca.status as details
FROM campaign_applications ca
JOIN campaigns c ON ca.campaign_id = c.id
JOIN profiles p ON ca.influencer_id = p.id
